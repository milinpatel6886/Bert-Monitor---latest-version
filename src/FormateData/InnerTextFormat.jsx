...
//   // Pattern 1: Header/Summary format e.g., GOLD, SILVER, INR with prices and L/H
//   const headerRegex =
...
//   // Pattern 2: ARIHANT-style multiline format with Buy, Low, Sell & High
//   const arihantRegex =
//     /((?:GOLD|SILVER).*?)(?:\t|\s+)(-|\d+\.?\d*)\s*\nL\s*:\s*(-|\d+\.?\d*)\s*\n(?:\t|\s+)(-|\d+\.?\d*)\s*\nH\s*:\s*(-|\d+\.?\d*)/gi;
...
//   // Pattern 3: COSTING/CURRENT/INDIA format with four prices (Buy, Sell, High, Low)
//   const costingRegex =
...
//   // Pattern 4: AUGMONT style with time and reversed Buy/Sell
//   const augmontRegex =
//     /((?:GOLD|SILVER).*?)\t(\d+\.?\d*)\s*\nHigh\s*-\s*(\d+\.?\d*)\s*\n\t(\d+\.?\d*)\s*\nLow\s*-\s*(\d+\.?\d*)\s*\n\t(\d{2}:\d{2}:\d{2})/gi;
...
//   // Pattern 5: ARYA/KUBERAN with date (delivery)
//   const deliveryRegex =
...
//     let buy = DEFAULT_RATE;
//     let sell = DEFAULT_RATE;

//     if (sourceName === "ARYA BULLION") {
//       buy = DEFAULT_RATE;
//       sell = rate1;
//     } else {
//       buy = rate1;
...
//   // Pattern 7: JR Bullion style with multiple prices in a line
//   // const bullionRegex =
//   //   /([A-Z0-9+() ]+(?:GOLD|SILVER)[A-Z0-9+() ]*(?:RAIPUR|MUMBAI)[A-Z0-9+() ]*)\s*\n\s*(-{2}|\d+)\s+(-{2}|\d+)\s+H-\s*(\d+)\s+L-\s*(\d+)/gi;
...
//   // Ensure all rows have all columns in the defined order and filter rows with valid Symbol Name
//   return results
//     .map((row) => {
//       const fullRow = {};
